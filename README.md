...
- **异步分析流程**：分析任务提交后进入 Redis 队列，由独立 Worker 消费执行，WebSocket 实时推送状态（自动降级轮询）。
- **AI 排障结果**：通过 DeepSeek API 生成摘要、异常原因和排障建议，结果以 JSON 数组存储，前端列表展示。
- **搜索命中高亮**：关键词筛选后，日志条目中命中文本自动高亮。
- **多维度筛选**：支持按关键词、状态、级别、服务/模块、时间范围筛选，时间选择器可视化。
- **统计图表**：排障面板展示级别分布和服务/模块分布的 CSS 柱状图。
- **告警规则**：自定义告警条件（级别/关键词/服务/阈值），一键评估当前日志是否触发告警。
- **日志列表分页**：后端分页查询，避免大数据量时页面卡顿。
- **上传进度条**：实时显示上传百分比，大文件上传体验友好。
- **导出功能**：分析结果导出为 .txt，日志条目导出为 .tsv。
- **深色模式**：CSS 变量化主题系统，一键切换，跟随系统偏好，localStorage 持久化。
- **前端组件化**：App.vue 拆分为 AuthPage、WorkspaceView、DetailView、ResponseView 四个独立组件。
...
- 日志列表支持按关键词、状态、级别、服务/模块、时间范围筛选
- 日志列表分页（20 条/页，后端分页查询）
- 日志详情支持按关键词、级别、服务/模块筛选
- 搜索命中高亮：关键词筛选后日志条目自动高亮匹配文本
- 时间筛选使用可视化日期时间选择器（`datetime-local`）
...
- 用户自定义告警规则：名称、级别条件、关键词条件、服务条件、触发阈值
- 规则按用户隔离，支持启用/禁用
- 一键评估当前日志是否触发已配置的告警规则
- 前端展示评估结果：触发/未触发，匹配条数与阈值对比

### 界面体验

- 深色模式：CSS 变量化主题系统，侧边栏一键切换，跟随系统偏好
- 上传进度条：实时显示上传百分比
- 导出功能：分析结果导出 .txt，日志条目导出 .tsv
- 前端组件化：AuthPage、WorkspaceView、DetailView、ResponseView 四个独立页面组件
...
| 日志列表与详情 | 已完成 | 支持账号隔离、列表分页、多维筛选、搜索高亮、级别筛选 |
| 真实注册登录 | 已完成 | 用户表、重复邮箱检查、密码哈希、JWT、Cookie 会话恢复、refresh token 续期 |
| AI 分析 | 已完成 | 摘要、异常原因、排障建议、历史记录、AI Key 缺失提示、JSONB 结构化存储 |
| 异步分析 | 已完成 | Redis 队列、独立 Worker、WebSocket 推送、自动降级轮询 |
| 展示型结果页 | 已完成 | 高频异常、关键信息聚合、关键事件、统计图表、截图 |
| 上传安全 | 已完成 | 文件大小/类型限制、空文件检测、编码检测、路径穿越防护、上传进度条 |
| 统一错误处理 | 已完成 | 全局异常捕获、一致错误响应、前端友好提示 |
| 告警规则 | 已完成 | 自定义规则、CRUD 接口、评估触发 |
| 生产配置 | 已完成 | CORS/Cookie/日志级别可配置、反向代理示例 |
| 深色模式 | 已完成 | CSS 变量主题、跟随系统、localStorage 持久化 |
| 导出功能 | 已完成 | 分析结果导出 .txt、日志条目导出 .tsv |
| 数据库迁移 | 已完成 | Alembic 管理表结构，Docker 启动自动迁移 |
| 前端测试 | 已完成 | Vitest 24 个测试用例覆盖 composables |
...
| 写入 10 万条到 PostgreSQL | 1.33s | 批量 INSERT（executemany），较逐条写入提速 3.5x |
| 分页查询（第 1000 页） | 0.04s | OFFSET/LIMIT 分页，50 条/页 |
| 统计聚合（级别/服务/趋势） | 0.09s | GROUP BY 聚合，包含趋势时间桶 |
| 日志列表查询 | 0.007s | 单用户日志列表 |
| 关键词搜索（ILIKE） | 0.08s | 在 10 万条中模糊搜索，命中 20325 条 |
| **端到端总耗时** | **1.88s** | 解析 + 写入 + 查询 |

### 说明

- 10 万行日志从上传到可检索不到 2 秒，使用 `executemany` 批量写入较逐条插入提速约 3.5 倍。
